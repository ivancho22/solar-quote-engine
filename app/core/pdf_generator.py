...
class PDFQuote(FPDF):
    def header(self):
        ...
        self.cell(0, 12, "PROPUESTA TÉCNICA - SISTEMA SOLAR ALL-IN-ONE", ln=True, align="C")
        self.set_font("Helvetica", "", 10)
        self.set_text_color(56, 189, 248) # Cyan
        self.cell(0, 4, "Dimensionamiento Preliminar y Estimación Económica", ln=True, align="C")
        self.ln(12)
    ...
    def footer(self):
        ...
        self.cell(0, 10, "Documento de estimación técnica preliminar. Sujeto a visita técnica en sitio.", align="C")
...
def generate_quote_pdf(client_data: dict, quote_data: dict) -> bytes:
    ...
    pdf.cell(0, 8, "1. INFORMACIÓN DEL CLIENTE Y UBICACIÓN", ln=True)
    ...
    pdf.cell(100, 6, f"Teléfono: {client_data.get('phone', 'N/A')}", ln=False)
    ...
    pdf.cell(0, 6, f"  - Batería Litio LiFePO4: {quote_data['cabinet']['battery_capacity_kwh']} kWh", ln=True)
    pdf.cell(0, 6, f"  - Dimensiones Gabinete: {quote_data['cabinet']['dimensions_cm']}", ln=True)
    pdf.cell(0, 6, f"• Generación Mensual Estimada: ~{quote_data['estimated_monthly_generation_kwh']} kWh/mes", ln=True)
    pdf.ln(6)

    # 3. Estimación Económica
    pdf.set_font("Helvetica", "B", 12)
    pdf.set_text_color(15, 23, 42)
    pdf.cell(0, 8, "3. ESTIMACIÓN FINANCIERA Y RETORNO", ln=True)
    pdf.line(10, pdf.get_y(), 200, pdf.get_y())
    pdf.ln(4)
    ...
    pdf.cell(0, 6, f"Rango de Inversión Llave en Mano: ${quote_data['total_cost_min_cop']:,.0f} - ${quote_data['total_cost_max_cop']:,.0f} COP", ln=True)
    pdf.set_x(14)
    pdf.set_font("Helvetica", "", 10)
    pdf.set_text_color(50, 50, 50)
    pdf.cell(0, 6, f"Ahorro mensual proyectado en factura: ~${quote_data['estimated_monthly_savings_cop']:,.0f} COP/mes", ln=True)
    pdf.set_x(14)
    pdf.cell(0, 6, f"Tiempo estimado de Retorno de Inversión (ROI): ~{quote_data['estimated_roi_years']} anos", ln=True)
